...
//Drive Definition
// Wrap the VEX drivetrain in a small class so we can override driveFor
// in one place. Edit MyDrivetrain::driveFor(...) below to change behavior.
...
void move_forward(double inches) { // Move backward for a distance in inches
  ...
  // to make it work and move for inches length, create a variable that is a placeholder for the inches covered
...
void move_backward(double inches) { // Move forward for a distance in inches
  Drivetrain.setDriveVelocity(80, percent);
  // double input_per_inch = 8 / 35; // Placeholder variable for distance
  // inches = inches * input_per_inch; // Adjust inches by the placeholder factor
  Drivetrain.driveFor(forward, inches, distanceUnits::in);
}
// void turn_right(int degrees_input) { // Turn left for a certain angle
// }
// void turn_left(int degrees) { // Turn right for a certain angle
//   Drivetrain.setTurnVelocity(80, percent);
//   Drivetrain.turnFor(right, degrees, rotationUnits::deg);
// }

void turnToRotate(int rotationAngle) {
  DrivetrainInertial.rotation(degrees);
  DrivetrainInertial.setRotation(0, degrees);
  Controller1.Screen.clearLine(3);
  Controller1.Screen.print(DrivetrainInertial.rotation(degrees));
  wait(1, seconds);
  // turn right
  int rotationA = rotationAngle - 1, rotationB = rotationAngle + 1;
  while(DrivetrainInertial.heading(degrees) <= rotationA || DrivetrainInertial.heading(degrees) >= rotationB) {
    // Turn here
    Controller1.Screen.clearScreen();
    Controller1.Screen.setCursor(1, 1);
    Controller1.Screen.print(DrivetrainInertial.heading());
    Controller1.Screen.setCursor(2, 1);
    if(DrivetrainInertial.heading() > rotationAngle) {
      Left.spin(forward);
      Right.spin(reverse);
      if(DrivetrainInertial.heading() > (rotationAngle - 1) && DrivetrainInertial.heading() < (rotationAngle + 1)) {
        Left.stop();
        Right.stop();
        break;
      }
    } else if(DrivetrainInertial.heading() < rotationAngle) {
      Left.spin(reverse);
      Right.spin(forward);
      if(DrivetrainInertial.heading() > (rotationAngle - 1) && DrivetrainInertial.heading() < (rotationAngle + 1)) {
        Left.stop();
...
//   move_backward(23);
//   // turnToRotate(270);
//   // intake_and_move_forward(48);
//   turnToRotate(0); // Heading 0
//   move_backward(47);
//   turnToRotate(270);
//   move_backward(33);
//   Controller1.Screen.clearScreen();
...
void autonomous(void) {
  Left.setVelocity(20, percent);
  Right.setVelocity(20, percent);
  turnToRotate(90);
  move_backward(8);
  // should be able to park in.
}
/*---------------------------------------------------------------------------*/
/*                                                                           */
/*                              User Control Task                            */
